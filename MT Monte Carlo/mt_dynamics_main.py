    ...
    def simulate(self, save=False):
        ...
        for j in range(1,self.Nsteps):
            ...
            for i in range(self.Nmtmax): # going through microtubules
                ...
                self.Kc[:,j][i]=cat_freq_update(self.Tub[j-1]) # catastophic frequency
...
#    seed_num=np.random.randint(0,100000)
    seed_num = 96152 # Nmtmax=20
    # seed_num=80532 # Nmtmax=80
    np.random.seed(seed_num)
    print('random seed '+str(seed_num))
    
    
    ms = MtSimulation()
    X_position, Rcell, LLmt = ms.simulate(save=False)  
...
#    ms.av_velocity() # Average velocity of MTs Vs time
    ...
    t=20
    plot_bundle(t,X_position,LLmt,xrange=0,Rcell=Rcell,label=True) # ploting MT distribution with bundles spread into xrange (xrange=0 because initial state has no MT bundle)
