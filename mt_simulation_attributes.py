# -*- coding: utf-8 -*-
"""
Created on Fri Feb 15 10:15:54 2019
...
def velocity_g(force,Tub): ## growth velocity
    c2=0.005 #μm s-1 μM-1
    return c2*Tub*np.exp(-force)
   
def cat_freq_update(vg,Tub):
    c3=0.04 #s-1
    c4=0.1 # μM-1
    kc = c3*np.exp(-c4*Tub)
    return kc
...
def force_component(X,crit_angle):
    Theta = []
    for i in range(len(X)):
        if X[i]<=X[0]+crit_angle:
            Theta.append(1)
        elif X[i]>=(X.max()-crit_angle):
            Theta.append(1)
        else:
            Theta.append(0)
    return Theta
